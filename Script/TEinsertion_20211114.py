...
insertion=insertion.loc[(insertion["Chr_start_y"]-insertion["Chr_end_x"]<=1000) & (insertion["Chr_start_y"]-insertion["Chr_end_x"]>=-1000)]
...
def re_countDouble(double_df):
	...
	I=f[["Chr_x","junctionStart","junctionEnd","Chrstrand_x","read","TE","TE_length_x","TE_start","TE_end","TEstrand_x","read_length_y","read-Chr-start_x","read-Chr-end_x","read-TE-start_x","read-TE-end_x","read-Chr-start_y","read-Chr-end_y"]]
	...
	I["j"]="double"
	print(I[0:10])
	I=I[["Chr_x","junctionStart","junctionEnd","Chrstrand_x","read","TE","j","Start_cluster","End_cluster","TE_length_x","TE_start","TE_end","TEstrand_x","read_length_y","read-Chr-start_x","read-Chr-end_x","read-TE-start_x","read-TE-end_x","read-Chr-start_y","read-Chr-end_y"]]
	I.columns=["Chr","junctionStart","junctionEnd","Chrstrand","read","TE","j","Start_cluster","End_cluster","TE_length","read_length","TE_start","TE_end","TEstrand","read-Chr-start","read-Chr-end","read-TE-start","read-TE-end","read-Chr-start_y","read-Chr-end_y"]
	return I
...
def Re_countSingle(orgFile):
	...
	f.loc[(f["Chrstrand"]=="-")&(f["dis1"]<=100),"junctionEnd"]=f["Chr_end"]
	f.loc[(f["Chrstrand"]=="-")&(f["dis2"]<=100),"junctionEnd"]=f["Chr_start"]
	f.loc[(f["Chrstrand"]=="+")&(f["dis1"]<=100),"junctionEnd"]=f["Chr_start"]
	f.loc[(f["Chrstrand"]=="+")&(f["dis2"]<=100),"junctionEnd"]=f["Chr_end"]
	...
	f["TEdir"]=False
	f.loc[(f["TE_start"]<=100) & (f["TE_end"]>=f["TE_length"]-100),"TEdir"]=True
	f.loc[(f["min"]==f["read-TE-start"]) & (f["TEstrand"]=="+")& (f["TE_end"]>=f["TE_length"]-100),"TEdir"]=True
	f.loc[(f["min"]==f["read-TE-start"]) & (f["TEstrand"]=="-")& (f["TE_start"]<=100),"TEdir"]=True
	f.loc[(f["min"]==f["read-Chr-start"])& (f["TEstrand"]=="+")&(f["TE_start"]<=100),"TEdir"]=True
	f.loc[(f["min"]==f["read-Chr-start"])& (f["TEstrand"]=="-")&(f["TE_end"]>=f["TE_length"]-100),"TEdir"]=True
	f=f.loc[f["TEdir"]==True]
	f=f.loc[(f["min"]<=100) & (f["max"]>=f["read_length"]-100)]
	...
	f["read-Chr-end_y"]=0
	I=f[["Chr","junctionStart","junctionEnd","Chrstrand","read","TE","j","Start_cluster","End_cluster","TE_length","TE_start","TE_end","TEstrand","read_length","read-Chr-start","read-Chr-end","read-TE-start","read-TE-end","read-Chr-start_y","read-Chr-end_y"]]
	return I

I1=Re_countSingle(pName+"_org_single.tsv")

if insertion.shape[0]>0:
